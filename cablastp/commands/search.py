"""cablastp-search CLI (port of cmd/cablastp-search/main.go)."""
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--blastp", default="blastp",
                        help="Location of the 'blastp' executable.")
...
def _blast_coarse(args, db, query: bytes) -> bytes:
    ...
    if proc.returncode != 0:
        raise RuntimeError(
            "blastp failed: %s" % err.decode("utf-8", errors="replace")
        )
    return out
